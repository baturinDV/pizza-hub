  ...
  const totalPrice = totalAmount + DELIVERY_PRICE + vatPrice;
    return (
        <WhiteBlock className="p-6 sticky top-4">
            ...
            } value={loading ? <Skeleton className="w-24 h-6 rounded-[6px]" /> : `${totalAmount} ₽`}/>
            <CheckoutItemDetails title={
                <div className="flex items-center">
                {VAT}<Percent size={18} className="mr-2 text-gray-400" />
                Сервисный сбор: 
                </div >
            } value={loading ? <Skeleton className="w-24 h-6 rounded-[6px]" /> : `${vatPrice} ₽`}/>
            <CheckoutItemDetails title={
                <div className="flex items-center">
                <Truck size={18} className="mr-2 text-gray-400" />
                Стоимость доставки: 
                </div >
            } value={loading ? <Skeleton className="w-24 h-6 rounded-[6px]" /> : `${DELIVERY_PRICE} ₽`}/>
